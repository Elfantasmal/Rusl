                        ...
                        <form method="POST" action={{ route('stocks.update', $stock->id) }}>
                            ...
                            <div class="box-body">
                                <div class="input-group">
                                    ...
                                    <select class="form-control  select2" name="commodity_id" style="width: 100%" disabled>
                                        <option value=""></option>
                                        @foreach($commodities as $id => $name)
                                            <option value="{{$id}}" {{ $id == $stock->commodity_id ? 'selected' : ''}}>
                                                {{$name}}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="input-group">
                                    ...
                                    <input id="company_phone" name="stock" type="text" class="form-control"
                                           placeholder="现有库存数量" value="{{ $stock->stock }}">
                                </div>
                                <br>
                                <div class="input-group">
                                    ...
                                    <input id="contact_name" name="stock_alert" type="text" class="form-control"
                                           placeholder="预警数量" value="{{ $stock->stock_alert }}">
                                </div>
                                <!-- /input-group -->
                            </div>
                            <!-- /.box-body -->
